...
export function PaymentsPage() {
    ...
    return (
        <div className="p-6 text-white">
            ...
            <div className="mb-4">
                <Link to="/platnosci/dodaj" className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded inline-flex items-center">
            ...
            {payments.length === 0 ? (
                <div className="bg-gray-800 p-10 rounded-lg text-center flex flex-col items-center shadow-lg">
                    ...
                    <p className="text-gray-400 max-w-md">
                        Nie znaleziono żadnych płatności. Dodaj pierwszą płatność, aby rozpocząć.
                    </p>
                </div>
            ) : (
                    ...
                    <table className="min-w-full leading-normal">
                        <thead>
                            <tr>
                                ...
                                <th className="px-5 py-3 border-b-2 border-gray-700 bg-gray-700 text-left text-xs font-semibold text-gray-300 uppercase tracking-wider">Numer Faktury</th>
                                ...
                                <th className="px-5 py-3 border-b-2 border-gray-700 bg-gray-700 text-left text-xs font-semibold text-gray-300 uppercase tracking-wider">Akcje</th>
                            </tr>
                        </thead>
                        <tbody>
                            {payments.map((payment) => (
                                <tr key={payment.id} className="hover:bg-gray-700">
                                    ...
                                    <td className="px-5 py-5 border-b border-gray-700 bg-gray-800 text-sm">
                                        <Link to={`/invoices/${payment.invoiceId}`} className="text-blue-400 hover:underline">
                                    ...
                                    <td className="px-5 py-5 border-b border-gray-700 bg-gray-800 text-sm">
                                        <Link to={`/platnosci/edytuj/${payment.id}`} className="text-yellow-500 hover:text-yellow-600 mr-3">
                                            <PencilIcon className="w-5 h-5 inline" />
                                        </Link>
                                        <button onClick={() => handleDeletePayment(payment.id)} className="text-red-500 hover:text-red-600">
